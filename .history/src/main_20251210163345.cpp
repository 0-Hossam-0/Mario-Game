...
void keyboard(unsigned char key, int x, int y)
{
    ...
    else if (currentState == STATE_GAME)
    {
        ...
        else
        {
            ...
            // Fireball for Luigi (Enter)
            if (key == 13 && luigi != nullptr && gameMode == 0)
            {
                luigi->shootFireball();
            }
        }
...
void specialKeys(int key, int x, int y)
{
    if (currentState == STATE_GAME && gameMode == 0 && luigi != nullptr)
    {
        ...
        {
        case GLUT_KEY_LEFT:
            luigi->setKeyState('a', true); 
            break;
        case GLUT_KEY_RIGHT:
            luigi->setKeyState('d', true); 
            break;
        case GLUT_KEY_UP:
            luigi->setKeyState('w', true); 
            break;
        case GLUT_KEY_DOWN:
            luigi->setKeyState('s', true); 
            break;
        }
    }
}
