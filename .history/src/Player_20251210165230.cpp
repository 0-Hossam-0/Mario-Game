...
{
  // Plays the specific mp3 requested
...
// UPDATED: Now plays sound when losing a life
void Player::loseLife()
{
  if (hud != nullptr)
  {
    // Only play sound if we actually have lives to lose
    if (hud->getLives() > 0)
    {
      hud->loseLife();
      playDamageSound(); // Trigger Sound
    }
  }
}
